...
train_img_files = sorted(glob.glob('../input/petfinder-adoption-prediction/train_images/*.jpg'),
                         key=lambda x: int(x.split('-')[-1].split('.')[0]))
test_img_files = sorted(glob.glob('../input/petfinder-adoption-prediction/test_images/*.jpg'),
                        key=lambda x: int(x.split('-')[-1].split('.')[0]))
...
print("Train shape {}, test shape {} after adding sentiment and metadata features".format(train_proc.shape,
                                                                                          test_proc.shape))
...
train_proc, test_proc = mean_encoding_kfold(train_proc, test_proc, ["Breed1", "Color1", "State"], "AdoptionSpeed",
                                            alpha=25)
...
categorical_columns_a = ['Type', 'Breed1', 'Breed2', 'Gender', 'Color1', 'Color2', 'Color3', 'MaturitySize',
                         'FurLength',
...
df_an_train = pd.DataFrame(
    {'PetID': train['PetID'].values, 'Pred_a': train_predictions_a.astype(np.int32), 'Score_a': train_scores_a})
df_an_train.to_csv('train_an.csv', index=False)

df_ha_train = pd.DataFrame(
    {'PetID': train['PetID'].values, 'Pred_h': train_predictions_h.astype(np.int32), 'Score_h': train_scores_h})
...
qwk_blend_simple = quadratic_weighted_kappa(X_train_a['AdoptionSpeed'].values,
                                            df_blend_train['AdoptionSpeed_Simple'].values)
print('QWK blend simple: {}'.format(qwk_blend_simple))

# generate submission
df_an_submission = pd.DataFrame(
    {'PetID': test['PetID'].values, 'Pred_a': test_predictions_a.astype(np.int32), 'Score_a': test_scores_a})
df_an_submission.to_csv('submission_an.csv', index=False)

df_ha_submission = pd.DataFrame(
    {'PetID': test['PetID'].values, 'Pred_h': test_predictions_h.astype(np.int32), 'Score_h': test_scores_h})
